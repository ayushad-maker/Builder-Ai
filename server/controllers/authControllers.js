...
export const register = async (req, res) => {
  ...
  if (!name || !email || !password) {
    res.status(400).json({ error: "Name,email and password is not defined." });
    return;
  }
  ...
  if (existing) {
    res
      .status(400)
      .json({ error: "An account with this email is already exits." });
    return;
  }
...
export const login = async (req, res) => {
  ...
  if (!email || !password) {
    res.status(400).json({ error: "Email and password is not defined." });
    return;
  }
